...
export const CategoryRepository = {
  ...
  update: (id: string, category: Partial<Category>) => db.categories.update(id, category),
...
export const TransactionRepository = {
  ...
  update: (id: string, transaction: Partial<Transaction>) => db.transactions.update(id, transaction),
...
export const BudgetRepository = {
  ...
  upsert: async (budget: Budget) => {
    const existing = await db.budgets
      .where({ year: budget.year, month: budget.month, categoryId: budget.categoryId })
      .first();
    if (existing) {
...
export const RecurringRepository = {
  ...
  update: (id: string, rule: Partial<RecurringRule>) => db.recurringRules.update(id, rule),
